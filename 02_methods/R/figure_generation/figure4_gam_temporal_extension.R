...
infile <- file.path(nm2_root, "04_results", "tables", "gam_pool_vs_strat_all.tsv")
outdir <- file.path(nm2_root, "04_results", "figure4b_gam_temporal_extension")
...
dt[, quadrant := fifelse(
  ...
  fifelse(
    ...
    fifelse(
      abs_delta_pool > x_cut & instability_score >= y_cut, "complex_temporal_structure",
      "background_weak"
    )
  )
)]
...
quad_colors <- c(
  ...
  "complex_temporal_structure" = "grey32",
...
fwrite(tier_summary, file.path(outdir, "figureS4b_gam_tier_summary.tsv"), sep = "\t")
...
p_left <- ggplot(dt, aes(x = abs_delta_pool, y = instability_score)) +
  ...
  annotate(
    ...
    label = "Pooling across time can obscure\ntemporally structured responses",
    hjust = 1,
    vjust = 1,
    color = "#B22222",
    size = 3.4,
  ...
  labs(
    title = "GAM: pooled effect vs instability",
...
p_right <- ggplot(tier_dt, aes(x = tier, y = hetero_gap)) +
  ...
  labs(
    title = "High-instability GAM genes \n show stronger temporal divergence",
    x = NULL,
    y = "Temporal divergence"
  ) +
...
try(
    ggsave(
      file.path(outdir, "figureS4b_gam_temporal_extension_locked.png"),
      final_plot,
      width = 9.4,
      height = 4.9,
      dpi = 400
...
ggsave(
  file.path(outdir, "figureS4b_gam_temporal_extension_locked.pdf"),
  final_plot,
  width = 9.4,
  height = 4.9,
  device = "pdf"
...
fwrite(summary_dt, file.path(outdir, "figureS4b_gam_summary.tsv"), sep = "\t")

cat("Figure S4B (GAM temporal extension) generated.\n\n")
print(summary_dt)
